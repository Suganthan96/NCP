...
export default function RedeemPermissionButton() {
  ...
  const handleRedeemPermission = async () => {
    if (!permission || !chain || !publicClient || !smartAccount) return;
    setLoading(true);
    ...
    try {
      ...
      /**
       * Sends a user operation with delegation to the bundler client. Only the smart account can redeem the delegation.
       * This operation includes:
       * - A transfer of 0.001 ETH to the specified recipient address
      ...
      const hash = await bundlerClient.sendUserOperationWithDelegation({
        ...
        calls: [
          {
            to: "0x9E239687ED8Fd4d79C781cA408E12bd209BC7762",
            data: '0x',
